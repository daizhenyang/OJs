...
public:
    double getAverage(vector <string> Rs) {
        string tot = "";
        FOREACH(Rs, i) tot += *i;
        vector<int> rating = reader(tot);
        int first = rating[0];
        sort(rating.begin(), rating.end(), greater<int>());
        int n = rating.size();
        int R = (n + 19) / 20;
        while(rating.size() % R != 0) rating.push_back(0);
        n = rating.size();
        int group = n / R;
        memset(e, 0, sizeof(e));
        for(int i = 0;i < group;i++) {
            int beg = i * R, end = (i + 1) * R;
            bool in = false;
            for(int j = beg;j < end;j++) if(rating[j] == first) in = true;
            if(in) {
                e[i + 1] += (e[i] * i + first) / (i + 1);
            } else {
                for(int j = beg;j < end;j++) {
                    if(rating[j] != 0) {
                        e[i + 1] += (e[i] * i + rating[j]) / (i + 1) * 1.0 / R;
                    } else {
                        e[i + 1] += e[i] * 1.0 / R;
                    }
                }
